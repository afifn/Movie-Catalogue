package com.kuycoding.jetpack.utils;

import com.kuycoding.jetpack.data.source.local.entity.MovieEntity;
import com.kuycoding.jetpack.data.source.local.entity.TvEntity;

import java.util.ArrayList;

public class FakeDataDummy {
    private static String baseImageURL = "https://image.tmdb.org/t/p/w500";

    public static ArrayList<MovieEntity> generateDataMovie() {
        ArrayList<MovieEntity> movies = new ArrayList<>();

        movies.add(new MovieEntity(
                "121.121",
                "7.5",
                baseImageURL + "/dzBtMocZuJbjLOXvrl4zGYigDzh.jpg",
                "a1",
                "false",
                baseImageURL + "/p3TCqUDoVsrIm8fHK9KOTfWnDjZ.jpg",
                "English",
                "Ad Astra",
                "Hi",
                "Ad Astra",
                "121",
                "The near future, a time when both hope and hardships drive humanity to look to the stars and beyond. While a mysterious phenomenon menaces to destroy life on planet Earth, astronaut Roy McBride undertakes a mission across the immensity of space and its many perils to uncover the truth about a lost expedition that decades before boldly faced emptiness and silence in search of the unknown.",
                "2019-09-17"
        ));
        return movies;
    }
    public static ArrayList<TvEntity> generateDummyTvShow() {
        ArrayList<TvEntity> tvshow = new ArrayList<>();
        tvshow.add(new TvEntity(
                "121.121",
                "7.5",
                baseImageURL + "/dzBtMocZuJbjLOXvrl4zGYigDzh.jpg",
                "a1",
                "false",
                baseImageURL + "/p3TCqUDoVsrIm8fHK9KOTfWnDjZ.jpg",
                "English",
                "Ad Astra",
                "Hi",
                "Ad Astra",
                "121",
                "The near future, a time when both hope and hardships drive humanity to look to the stars and beyond. While a mysterious phenomenon menaces to destroy life on planet Earth, astronaut Roy McBride undertakes a mission across the immensity of space and its many perils to uncover the truth about a lost expedition that decades before boldly faced emptiness and silence in search of the unknown.",
                "2019-09-17"
        ));
        return tvshow;
    }

    public static MovieEntity getMovie(String movieId) {
        for (int i = 0;i < generateDataMovie().size();i++) {
            MovieEntity entity = generateDataMovie().get(i);
            if (entity.getId().equals(movieId)){
                return entity;
            }
        }
        return null;
    }

    public static TvEntity getTvShow(String movieId) {
        for (int i = 0; i < generateDummyTvShow().size();i++) {
            TvEntity entity = generateDummyTvShow().get(i);
            if (entity.getId().equals(movieId)){
                return entity;
            }
        }
        return null;
    }

    public static MovieEntity generateDummyMovie(MovieEntity course, boolean bookmarked) {
        MovieEntity courseWithModule = new MovieEntity();
//        courseWithModule. = course;
//        courseWithModule.mCourse.setFavorite(bookmarked);
//        courseWithModule.mCourse = getMovie(course.getId());
        return courseWithModule;
    }

    public static TvEntity generateDummyTv(TvEntity course, boolean bookmarked) {
        TvEntity courseWithModule = new TvEntity();
//        courseWithModule.mCourse = course;
//        courseWithModule.mCourse.setFavorite(bookmarked);
//        courseWithModule.mCourse = getTvShow(course.getId());
        return courseWithModule;
    }
}
